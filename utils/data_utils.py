...
def datafold_read_(args, report=None, plan_raw=None):
    ...
    # print(len(data))
    # print(args.p_data)
    tr = data[:int(p_tr*len(data)*args.p_data)]
    if args.test_mode == 1:
        val = data[int(p_val*len(data)):]
    elif args.test_mode >= 2:
        val = data
    else:
        val = data[int(p_tr*len(data)):int(p_val*len(data))]
...
def prepare_report(args, row, i=0):
    ...
    # # t & n stage   according to   icT & icN
    ...
    # # orientation according to subsite
    ...
    # # surgery type according to remark
    ...
    # Age    Male    T stage    N stage    M stage    Grade
    age = str(row['Age'].values[i])
    male = 'male' if row['Male'].values[i] == '1' else 'female'
    t_stage = 'T' + str(row['T stage'].values[i])
    n_stage = 'N' + str(row['N stage'].values[i])
    m_stage = 'M' + str(row['M stage'].values[i])
    grade = 'Grade' + str(row['Grade'].values[i])

    text_prompt = ', '.join([age, male, t_stage, n_stage, m_stage, grade])
    # print(text_prompt)
