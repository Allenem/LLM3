        ...
        for m, p in zip(img_size, patch_size):
            for i in range(5):
                ...
                if m % np.power(p, i + 1) != 0:
                    raise ValueError(f"input image size {img_size} should be divisible by stage-wise image resolution {np.power(p, i + 1)}.")
    ...
    def interactive_alignment(self, hidden_states_out, report_in, x_in):
        ...
        for j, text_vis in enumerate(zip(report_l[h_offset:], hidden_states_out[h_offset:])):
            ...
            if len(report_in) != len(x_in):
                txt = torch.repeat_interleave(txt, vis.shape[0], dim=0) # repeat text for each image
