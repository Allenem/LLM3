...
def main_worker(gpu, args):
    ...
    if not args.test_mode:
        print(f'Batch Cnt: Train: {len(loader[0])}, Val: {len(loader[1])}')
    else:
        print(f'Batch Cnt: Test: {len(loader[1])}')
        
    # load
    best_acc, start_epoch = 0, 0
    model.test = False
    
    if args.resume_ckpt & (args.test_mode == 0):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LLMSeg")
    ...
    parser.add_argument("--a_max", default=4095.0, type=float, help="a_max in ScaleIntensityRanged") 
    parser.add_argument("--b_min", default=0.0, type=float, help="b_min in ScaleIntensityRanged")
    parser.add_argument("--b_max", default=1.0, type=float, help="b_max in ScaleIntensityRanged")
    parser.add_argument("--c_max", default=2, type=int, help="label in ScaleIntensityRanged")
    ...
    parser.add_argument("--llama_rep", default="meta-llama/Llama-2-7b-chat-hf", type=str)
    ...
    parser.add_argument("--context_mode", default=0, type=int)  # {0: Esential, 1: Expanded, 2: Personalized}
    parser.add_argument("--ablation", default="", type=str)     # ["RL", "Swap", "None"]
    parser.add_argument("--compare_mode", default=0, type=int)  # {1: ConTEXTNet}
    
    args = parser.parse_args()

    # args.data_dir = ["./dataset/internal/"]
    # args.report_dir = ["./dataset/internal/interanl_report.xlsx"]
    args.data_dir = ["./dataset/train/"]
    args.report_dir = ["./dataset/train/report1.xlsx"]

    # if args.test_mode == 2:
    #     args.data_dir = ["./dataset/external1/"]
    #     args.report_dir = ["./dataset/external1/external1_report.xlsx"]
    if args.test_mode == 2:
        args.data_dir = ["./dataset/test/"]
        args.report_dir = ["./dataset/test/report2.xlsx"]

    # elif args.test_mode == 3:
    #     args.data_dir = ["./dataset/external2/"]
    #     args.report_dir = ["./dataset/external2/external2_report.xlsx"]
...
# python main.py --logdir ./ckpt/multimodal --save_checkpoint True --max_epochs 200 --context False
# python main.py --pretrained_dir ./ckpt/multimodal --context False                                 --test_mode 2 --batch_size 1 --save_interval 10